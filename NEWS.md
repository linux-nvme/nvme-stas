...
* **DIM registration with a Centralized Discovery Controller never actually ran.** `stafd` called a method that does not exist on libnvme's `nvme.Ctrl` (`registration_ctlr` instead of `registration_control`), so registering with a CDC per TP8010 silently never happened, with no error raised.
* **A connection with no entry of its own in `nvme-stas.conf` (mDNS-discovered, or learned from a Discovery Log Page) now correctly picks up the file's own `[Host]` section**, instead of consulting only the flat `/etc/nvme/hostnqn`/`hostid` files — which could abort the process on startup if a persona was configured but those files didn't exist. `hostid` is now also inherited from a Discovery Controller to the I/O controllers and referrals discovered through it, and a persona's own `hostnqn` with no `hostid` of its own recovers one automatically when the NQN is in `uuid:` form.
